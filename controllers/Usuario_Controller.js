...
class UsuarioController{
    ...
    registrar_usuario(nuevo_usuario){
      return new Promise((resolve, reject)=>{
        Usuario_model.guardar_usuario(nuevo_usuario).then(()=>{//token
          //resolve(token)
          resolve()
    ...
    modificar_usuario(id, actualizar){
      return new Promise((resolve, reject)=>{
        Usuario_model.modificar_usuario(id, actualizar).then(()=>{resolve()}).catch((error)=>{reject(error)})
      }) 
    }
    borrar_usuario(id){
      return new Promise((resolve, reject)=>{
        Usuario_model.eliminar_usuario(id).then(()=>{resolve()}).catch((error)=>{reject(error)})
      }) 
    }
}
